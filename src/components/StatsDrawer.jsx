  ...
  return (
    <>
      ...
      <div style={{ ...s.drawer, maxHeight: open ? '50vh' : 0 }}>
        <div style={s.inner}>
          {!stats ? (
            <div style={s.empty}>Close a position to see your performance stats.</div>
          ) : (
            <>
              <div style={s.grid}>
                <Card label="Win Rate"      value={`${stats.winRate.toFixed(1)}%`}
                      color={stats.winRate >= 50 ? C.green : C.red} />
                <Card label="Avg RR"        value={stats.avgRR != null ? stats.avgRR.toFixed(2) : '—'}
                      color={stats.avgRR != null && stats.avgRR >= 1 ? C.green : C.red} />
                <Card label="Profit Factor" value={isFinite(stats.profitFactor) ? stats.profitFactor.toFixed(2) : '∞'}
                      color={stats.profitFactor >= 1 ? C.green : C.red} />
...
const s = {
  ...
  inner:  { overflowY: 'auto', padding: '10px 12px 14px', borderTop: `1px solid ${C.border}` },
  empty:  { fontSize: 12, color: C.muted, textAlign: 'center', padding: '14px 0' },
  grid:   { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, marginBottom: 10 },
  card:   { background: C.bg, borderRadius: 6, padding: '8px 6px', textAlign: 'center' },
  cardLabel: { fontSize: 10, color: C.muted, marginBottom: 4 },
  cardValue: { fontSize: 15, fontWeight: 700 },
  curveWrap: { marginTop: 4 },
  curveLabel: { fontSize: 10, color: C.muted, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.8 },
}
